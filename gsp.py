...
def isSubsequenceRecursive(mainSequence, subSequenceClone, start=0):
    ...
    for i in tqdm.tqdm(range(start, len(mainSequence)), total=len(range(start, len(mainSequence))), desc="Iterating over customers", leave = False):
...
def countSupport(dataset, candidateSequence):
    #print(f"\nCOUNTSUPPORT\n dataset:{dataset}, cand:{candidateSequence}")
...
def apriori(dataset, minSupport, verbose=False):
    ...
    Overall = []
    itemsInDataset = sorted(set([item for sublist1 in dataset for item in sublist1]))
    print("itemsInDataset: ", itemsInDataset)
    #print("Alex")
    singleItemSequences = [[item] for item in itemsInDataset] # is usefull [item] (?)
    print("singleItemSequences: ", singleItemSequences)
    #exit()
    #computes 2 times recursive shit ggwp
    ...
    for i in singleItemSequences:
        x = countSupport(dataset, i)
        if x >= minSupport:
            singleItemCounts.append((i, x))
    #print("EP")
    Overall.append(singleItemCounts)
    if verbose:
        print("Result, lvl 1: " + str(Overall[0]))

    print("\nsingleItemCounts: ", singleItemCounts)
    return
